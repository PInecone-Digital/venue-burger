...
async function getData() {
  const response = await fetch("data/stacked.json", { cache: "no-store" });
  ...
  data.menu.forEach((category) => {
    ...
    category.items.forEach((item) => {
      ...
      output += `
        <div class="col-12 col-sm-6 col-md-6 col-lg-4 d-flex justify-content-between cursor-pointer" 
             data-bs-toggle="modal" 
             data-bs-target="#itemModal" 
             data-bs-image="${item.image ? `/img/menu/${item.image}` : "/img/cover-placeholder.jpg"}" 
             data-bs-name="${item.name}" 
             data-bs-description="${item.description}" 
             data-bs-price="${formattedPrice}">
          <div class="flex-grow-1 me-3">
            <div class="d-flex align-items-baseline">
              ${item.name ? `<p class="food-title mb-1">${item.name}</p>` : ""}
            </div>
            <p class="menu-price mb-1">${formattedPrice}</p>
            <p class="food-details">${item.description}</p>
          </div>
          <div class="flex-shrink-0">
            <img class="menu-item-image img-fluid bg-secondary" 
